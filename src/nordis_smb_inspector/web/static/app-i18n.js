"use strict";

const STATUS_LABELS = {
  port_open: "445 açık",
  timeout_no_response: "Yanıt yok / timeout",
  connection_refused: "Bağlantı reddedildi",
  network_unreachable: "Ağa ulaşılamıyor",
  connection_error: "Bağlantı hatası",
  dns_resolution_failed: "DNS çözümlenemedi",
  negotiation_failed: "SMB görüşmesi başarısız",
  smb1_only_unsupported: "Yalnız SMB1 destekleniyor",
  authenticated: "Doğrulandı",
  kerberos: "Kerberos",
  ntlm: "NTLM",
  ntlm_fallback_used: "NTLM fallback kullanıldı",
  auth_failed: "Kimlik doğrulanamadı",
  ntlm_fallback_unavailable: "NTLM fallback kullanılamıyor",
  access_denied: "Erişim reddedildi",
  share_enum_denied: "Share listesi reddedildi",
  share_enum_unavailable: "Share listesi alınamıyor",
  share_enum_failed: "Share keşfi başarısız",
  share_connected: "Share erişilebilir",
  share_access_denied: "Share erişimi reddedildi",
  non_file_share: "Dosya share'i değil",
  directory_listable: "Dizin listelenebilir",
  directory_list_denied: "Dizin listeleme reddedildi",
  directory_list_error: "Dizin listeleme hatası",
  entry_error: "Geçersiz dizin girdisi",
  depth_limit_reached: "Derinlik sınırına ulaşıldı",
  file_readable: "Dosya okunabilir",
  file_read_denied: "Dosya okuma reddedildi",
  sharing_violation: "Paylaşım ihlali",
  read_error: "Okuma hatası",
  partial_access: "Kısmi erişim",
  security_active_required: "Aktif · Zorunlu",
  security_active: "Aktif",
  security_required: "Zorunlu",
  security_supported: "Destekli",
  security_unsupported: "Desteklenmiyor",
  cancelled: "İptal edildi",
  completed: "Tamamlandı",
  failed: "Başarısız",
  wordlist: "Wordlist",
  pattern: "Kalıp",
  artifact: "Dosya sinyali",
  allowed: "İzin var",
  denied: "Reddedildi",
  unknown: "Bilinmiyor",
  error: "Hata",
  cleanup_failed: "Temizleme başarısız",
  // TargetStage degerleri: canli olayda status bos oldugunda last_status
  // alanina asama dusuyor (web/app.py: _inspection_event_payload).
  network: "Ağ katmanı",
  negotiation: "SMB görüşmesi",
  authentication: "Kimlik doğrulama",
  authorization: "Yetkilendirme",
  share_enumeration: "Share keşfi",
  tree_walk: "Dizin gezintisi",
  file_read: "Dosya okuma",
  complete: "Tamamlandı",
};
const FINDING_METHOD_LABELS = {
  wordlist: "Arama terimi",
  pattern: "Otomatik tespit",
  artifact: "Dosya sinyali",
};
const EN_FINDING_METHOD_LABELS = {
  wordlist: "Search term",
  pattern: "Automatic detection",
  artifact: "File signal",
};
const FINDING_RULE_LABELS = {
  "jwt-token": "JWT tokenı",
  "private-key-header": "Özel anahtar dosyası başlığı",
  "authorization-bearer": "Bearer erişim tokenı",
  "authorization-basic": "HTTP Basic kimlik doğrulama değeri",
  "credential-url": "URL'ye gömülü kimlik bilgisi",
  "connection-string-password": "Bağlantı dizesindeki veritabanı parolası",
  "secret-assignment": "Yapılandırmadaki parola, anahtar veya token",
  "recovery-secret-assignment": "Kurtarma kodu veya MFA sırrı",
  "authorization-token-header": "HTTP kimlik doğrulama başlığındaki token",
  "slack-token-prefix": "Slack erişim tokenı",
  "cookie-secret-assignment": "Oturum çerezi değeri",
  "netrc-credential": "netrc dosyasındaki kimlik bilgisi",
  "age-encrypted-file": "age ile şifrelenmiş dosya",
  "encrypted-pem-marker": "Şifreli PEM özel anahtarı",
  "xml-secret-element": "XML öğesindeki gizli değer",
  "url-query-secret": "URL sorgu parametresindeki gizli değer",
  "age-secret-key": "age gizli anahtarı",
  "gpp-cpassword": "Group Policy Preferences cpassword değeri",
  "kerberos-tgs-artifact": "Kerberos TGS verisi",
  "kerberos-asrep-artifact": "Kerberos AS-REP verisi",
  "kerberos-preauth-artifact": "Kerberos ön kimlik doğrulama verisi",
  "kerberos-db-key": "Kerberos KDC veritabanı anahtarı",
  "windows-nt-hash": "Etiketli NTLM hash'i",
  "windows-lm-hash": "Etiketli LM hash'i",
  "windows-password-history": "Windows parola geçmişi veya ek kimlik bilgisi",
  "kerberos-rc4-key": "Kerberos RC4 anahtarı",
  "kerberos-aes128-key": "Kerberos AES-128 anahtarı",
  "kerberos-aes256-key": "Kerberos AES-256 anahtarı",
  "kerberos-des-key": "Kerberos DES anahtarı",
  "windows-managed-password": "Windows yönetilen parola özniteliği",
  "windows-autologon-password": "Windows otomatik oturum açma parolası",
  "windows-unattend-password": "Windows katılımsız kurulum parolası",
  "powershell-plaintext-password": "PowerShell düz metin parola girdisi",
  "windows-lsa-secret": "Windows LSA veya DPAPI gizli değeri",
  "rdp-password-blob": "RDP korumalı parola değeri",
  "windows-command-password": "Windows komut satırındaki parola",
  "lm-nt-hash-pair": "LM/NT hash çifti",
  "credential-dump-line": "Hesap RID ve LM/NT hash kaydı",
  "netntlmv2-response": "NetNTLMv2 sınama-yanıt verisi",
  "netntlmv1-response": "NetNTLMv1 sınama-yanıt verisi",
  "dcc2-hash": "Önbelleğe alınmış etki alanı kimlik bilgisi (DCC2)",
  "labelled-password-hash": "Etiketli parola hash'i",
  "unix-password-hash": "Unix crypt parola hash'i",
  "modern-password-hash": "bcrypt veya Argon2 parola hash'i",
  "portable-php-password-hash": "phpass veya Drupal parola hash'i",
  "apache-apr1-hash": "Apache APR1 parola hash'i",
  "pbkdf2-password-hash": "PBKDF2 parola hash'i",
  "ldap-password-hash": "LDAP parola hash'i",
  "mysql-password-hash": "MySQL parola hash'i",
  "mssql-password-hash": "Microsoft SQL Server parola hash'i",
  "postgresql-scram-hash": "PostgreSQL SCRAM parola hash'i",
  "cisco-password-hash": "Cisco IOS parola hash'i",
  "cloud-access-key": "AWS erişim anahtarı kimliği",
  "github-token-prefix": "GitHub erişim tokenı",
  "github-fine-grained-token": "GitHub ayrıntılı erişim tokenı",
  "gitlab-token-prefix": "GitLab erişim tokenı",
  "stripe-secret-key": "Stripe gizli anahtarı",
  "stripe-webhook-secret": "Stripe webhook imzalama sırrı",
  "sendgrid-api-key": "SendGrid API anahtarı",
  "google-api-key": "Google API anahtarı",
  "huggingface-token-prefix": "Hugging Face erişim tokenı",
  "private-token-header": "Özel erişim tokenı başlığı",
  "aws-secret-access-key": "AWS gizli erişim anahtarı",
  "digitalocean-token-prefix": "DigitalOcean erişim tokenı",
  "azure-devops-pat": "Azure DevOps kişisel erişim tokenı",
  "azure-storage-account-key": "Azure Storage hesap anahtarı",
  "azure-sas-token": "Azure paylaşımlı erişim imzası",
  "google-oauth-client-secret": "Google OAuth istemci sırrı",
  "google-oauth-access-token": "Google OAuth erişim tokenı",
  "ai-provider-api-key": "Yapay zekâ servisi API anahtarı",
  "new-relic-api-key": "New Relic API anahtarı",
  "slack-webhook-url": "Slack webhook adresi",
  "discord-webhook-url": "Discord webhook adresi",
  "telegram-bot-token": "Telegram bot tokenı",
  "shopify-access-token": "Shopify erişim tokenı",
  "npm-token-prefix": "npm erişim tokenı",
  "pypi-token-prefix": "PyPI API tokenı",
  "vault-token-prefix": "Vault erişim tokenı",
  "docker-registry-auth": "Docker imaj kayıt deposu kimlik bilgisi",
  "ansible-vault-artifact": "Ansible Vault şifreli verisi",
  "sops-encrypted-artifact": "SOPS ile şifrelenmiş değer",
  "grafana-service-account-token": "Grafana servis hesabı tokenı",
  "npm-auth-token-assignment": "npm kayıt deposu kimlik doğrulama tokenı",
  "kubernetes-client-key-data": "Kubernetes istemci özel anahtarı",
  "terraform-cloud-token": "Terraform Cloud erişim tokenı",
  "hcp-terraform-api-token": "HCP Terraform API tokenı",
  "jenkins-api-token": "Jenkins API tokenı",
  "sonarqube-token-prefix": "SonarQube erişim tokenı",
  "docker-personal-access-token": "Docker kişisel erişim tokenı",
  "docker-organization-access-token": "Docker kuruluş erişim tokenı",
  "kerberos-ccache-file": "Kerberos kimlik bilgisi önbellek dosyası (CCache)",
  "credential-container-filename": "Credential kapsayıcısı dosya adı",
  "private-key-filename": "Özel anahtar dosya adı",
  "credential-related-filename": "Credential ilişkili dosya adı",
  "kerberos-keytab-file": "Kerberos anahtar tablosu dosyası (keytab)",
  "kerberos-kirbi-file": "Kerberos bilet dosyası (KIRBI)",
};
const EN_FINDING_RULE_LABELS = {
  "cloud-access-key": "Cloud access key",
  "jwt-token": "JWT session token",
  "private-key-header": "Private key header",
  "authorization-bearer": "Bearer token",
  "authorization-basic": "Basic authentication value",
  "credential-url": "Credential embedded in URL",
  "secret-assignment": "Password or token",
  "connection-string-password": "Database connection password",
  "gpp-cpassword": "Group Policy cpassword",
  "recovery-secret-assignment": "Recovery code or MFA secret",
  "authorization-token-header": "Authentication header token",
  "kerberos-tgs-artifact": "Kerberos TGS-REP material",
  "kerberos-asrep-artifact": "Kerberos AS-REP material",
  "kerberos-preauth-artifact": "Kerberos pre-auth material",
  "kerberos-db-key": "Kerberos KDC database key",
  "windows-nt-hash": "NTLM hash",
  "windows-lm-hash": "LM hash",
  "windows-password-history": "Windows password history or supplemental credential",
  "kerberos-rc4-key": "Kerberos RC4 key",
  "kerberos-aes128-key": "Kerberos AES-128 key",
  "kerberos-aes256-key": "Kerberos AES-256 key",
  "kerberos-des-key": "Kerberos DES key",
  "lm-nt-hash-pair": "LM/NT hash pair",
  "credential-dump-line": "Account RID/hash record",
  "netntlmv1-response": "NetNTLMv1 response",
  "netntlmv2-response": "NetNTLMv2 response",
  "dcc2-hash": "Cached domain credential",
  "labelled-password-hash": "Labelled password hash",
  "unix-password-hash": "Unix password hash",
  "modern-password-hash": "Bcrypt or Argon2 hash",
  "github-token-prefix": "GitHub access token",
  "github-fine-grained-token": "GitHub fine-grained token",
  "gitlab-token-prefix": "GitLab access token",
  "slack-token-prefix": "Slack token",
  "stripe-secret-key": "Stripe secret key",
  "stripe-webhook-secret": "Stripe webhook signing secret",
  "sendgrid-api-key": "SendGrid API key",
  "google-api-key": "Google API key",
  "npm-token-prefix": "npm access token",
  "pypi-token-prefix": "PyPI API token",
  "huggingface-token-prefix": "Hugging Face token",
  "vault-token-prefix": "Vault token",
  "private-token-header": "Private access token header",
  "cookie-secret-assignment": "Session cookie value",
  "netrc-credential": "netrc credential",
  "aws-secret-access-key": "AWS secret access key",
  "docker-registry-auth": "Docker registry credential",
  "ansible-vault-artifact": "Ansible Vault material",
  "sops-encrypted-artifact": "SOPS encrypted value",
  "windows-managed-password": "Windows managed password field",
  "age-encrypted-file": "Age encrypted file",
  "encrypted-pem-marker": "Encrypted PEM private key",
  "xml-secret-element": "Credential value in XML",
  "url-query-secret": "Secret in URL query",
  "age-secret-key": "Age secret key",
  "digitalocean-token-prefix": "DigitalOcean access token",
  "azure-devops-pat": "Azure DevOps personal access token",
  "azure-storage-account-key": "Azure Storage account key",
  "azure-sas-token": "Azure shared access signature",
  "google-oauth-client-secret": "Google OAuth client secret",
  "google-oauth-access-token": "Google OAuth access token",
  "ai-provider-api-key": "AI service API key",
  "new-relic-api-key": "New Relic API key",
  "slack-webhook-url": "Slack webhook URL",
  "discord-webhook-url": "Discord webhook URL",
  "telegram-bot-token": "Telegram bot token",
  "shopify-access-token": "Shopify access token",
  "grafana-service-account-token": "Grafana service account token",
  "npm-auth-token-assignment": "npm registry token",
  "kubernetes-client-key-data": "Kubernetes client private key",
  "terraform-cloud-token": "Terraform Cloud token",
  "hcp-terraform-api-token": "HCP Terraform API token",
  "jenkins-api-token": "Jenkins API token",
  "sonarqube-token-prefix": "SonarQube access token",
  "docker-personal-access-token": "Docker personal access token",
  "docker-organization-access-token": "Docker organization access token",
  "windows-autologon-password": "Windows automatic logon password",
  "windows-unattend-password": "Windows unattended setup password",
  "powershell-plaintext-password": "PowerShell plaintext password",
  "windows-lsa-secret": "Windows LSA or DPAPI secret",
  "rdp-password-blob": "RDP protected password value",
  "windows-command-password": "Windows command-line password",
  "portable-php-password-hash": "phpass or Drupal password hash",
  "apache-apr1-hash": "Apache APR1 password hash",
  "pbkdf2-password-hash": "PBKDF2 password hash",
  "ldap-password-hash": "LDAP password hash",
  "mysql-password-hash": "MySQL password hash",
  "mssql-password-hash": "Microsoft SQL Server password hash",
  "postgresql-scram-hash": "PostgreSQL SCRAM password hash",
  "cisco-password-hash": "Cisco IOS password hash",
  "kerberos-ccache-file": "Kerberos credential cache file",
  "credential-container-filename": "Credential container filename",
  "private-key-filename": "Private-key filename",
  "credential-related-filename": "Credential-related filename",
  "kerberos-keytab-file": "Kerberos keytab file",
  "kerberos-kirbi-file": "Kerberos ticket file",
};
const SCAN_STATUS_LABELS = {
  idle: "Tarama yok",
  running: "Çalışıyor",
  cancelling: "İptal ediliyor",
  cancelled: "İptal edildi",
  completed: "Tamamlandı",
  failed: "Başarısız",
};
const PHASE_LABELS = {
  preparing_targets: "Hedefler hazırlanıyor",
  connectivity: "TCP/445 kontrolü",
  inspection: "SMB ve içerik taraması",
  authentication: "Kimlik doğrulama",
  share_discovery: "Share keşfi",
  file_inventory: "Dosya envanteri",
  content_scan: "İçerik taraması",
  identity_access: "Kimlik erişimi",
  cancelling: "İptal ediliyor",
  cancelled: "İptal edildi",
  completed: "Tamamlandı",
  failed: "Başarısız",
};
const STATUS_MESSAGES = {
  idle: "Yeni bir tarama başlatılmadı.",
  cancelling: "Tarama iptal ediliyor.",
  cancelled: "Tarama iptal edildi.",
  completed: "Tarama tamamlandı.",
  failed: "Tarama başarısız.",
};
const MESSAGE_LABELS = {
  "Cancellation requested.": "İptal isteği gönderildi.",
  "Scan cancelled.": "Tarama iptal edildi.",
  "Scan worker failed.": "Tarama başarısız.",
};
const ERROR_MESSAGE_LABELS = {
  "The target refused the TCP connection.": "Hedef TCP bağlantısını reddetti.",
  "The local network stack reported that the target is unreachable.": "Hedef ağa ulaşılamıyor.",
  "No TCP response was received before the configured timeout.": "Süre dolmadan TCP yanıtı alınamadı.",
  "SMB authentication failed.": "SMB kimlik doğrulaması başarısız.",
  "The supplied credential was not accepted.": "Girilen kimlik bilgisi kabul edilmedi.",
  "The supplied credential has expired.": "Girilen kimlik bilgisinin süresi dolmuş.",
  "The account cannot connect to this share.": "Hesabın bu share'e erişimi reddedildi.",
  "The named share was not found.": "Belirtilen share bulunamadı.",
  "The directory could not be listed.": "Dizin listelenemedi.",
  "Directory listing was denied.": "Dizin listeleme reddedildi.",
  "The directory could not be listed completely.": "Dizin tamamen listelenemedi.",
  "One directory entry was invalid; the remaining entries were inspected.": "Bir dizin girdisi geçersizdi; kalan girdiler incelendi.",
  "The file is visible but read access was denied.": "Dosya görünüyor fakat okuma erişimi reddedildi.",
  "The visible file could not be opened for reading.": "Görünen dosya okumak için açılamadı.",
  "The target inspection completed with inaccessible content.": "Hedef incelemesi erişilemeyen içerikle tamamlandı.",
  "Share listing was unavailable; known share names were probed directly.": "Share listesi alınamadı; bilinen share adları doğrudan denendi.",
};
const LANGUAGE_KEY = "nordis.dashboard-language";
let currentLanguage = (() => {
  try {
    return localStorage.getItem(LANGUAGE_KEY) === "en" ? "en" : "tr";
  } catch (_error) {
    return "tr";
  }
})();
const EN_STATUS_LABELS = {
  port_open: "Port 445 open", timeout_no_response: "No response / timeout",
  connection_refused: "Connection refused", network_unreachable: "Network unreachable",
  authenticated: "Authenticated", auth_failed: "Authentication failed",
  access_denied: "Access denied", share_enum_denied: "Share listing denied",
  share_enum_unavailable: "Share listing unavailable", share_enum_failed: "Share discovery failed",
  partial_access: "Partial access", completed: "Completed", cancelled: "Cancelled",
  failed: "Failed",
  allowed: "Allowed", denied: "Denied", unknown: "Unknown", error: "Error",
  cleanup_failed: "Cleanup failed",
  wordlist: "Wordlist match", pattern: "Pattern match", artifact: "File signal",
  security_active_required: "Active · Required", security_active: "Active",
  security_required: "Required", security_supported: "Supported", security_unsupported: "Unsupported",
  share_connected: "Share accessible", share_access_denied: "Share access denied",
  non_file_share: "Non-file share", directory_listable: "Directory listable",
  directory_list_denied: "Directory listing denied", directory_list_error: "Directory listing error",
  entry_error: "Invalid directory entry", depth_limit_reached: "Depth limit reached",
  file_readable: "File readable", file_read_denied: "File read denied",
  sharing_violation: "Sharing violation", read_error: "Read error",
  network: "Network", negotiation: "SMB negotiation",
  authentication: "Authentication", authorization: "Authorization",
  share_enumeration: "Share discovery", tree_walk: "Directory walk",
  file_read: "File read", complete: "Completed",
};
const EN_CATEGORY_LABELS = {
  "Cloud / SaaS": "Cloud / SaaS", "Oturum tokenı": "Session token",
  "Kriptografik anahtar": "Cryptographic key", "Kimlik bilgisi": "Credential",
  Veritabanı: "Database", Yapılandırma: "Configuration", "Windows / AD": "Windows / AD",
  "Credential artifact": "Credential artifact", "Source control": "Source control",
  "Ödeme servisi": "Payment service", "Developer tooling": "Developer tooling",
  Infrastructure: "Infrastructure", "Container tooling": "Container tooling",
};
const TR_CATEGORY_LABELS = {
  "Cloud / SaaS": "Bulut / SaaS", "Oturum tokenı": "Oturum tokenı",
  "Kriptografik anahtar": "Kriptografik anahtar", "Kimlik bilgisi": "Kimlik bilgisi",
  Veritabanı: "Veritabanı", Yapılandırma: "Yapılandırma", "Windows / AD": "Windows / AD",
  "Credential artifact": "Kimlik doğrulama verisi", "Source control": "Kaynak kod yönetimi",
  "Ödeme servisi": "Ödeme servisi", "Developer tooling": "Geliştirici araçları",
  Infrastructure: "Altyapı", "Container tooling": "Konteyner araçları",
};
const EN_PHASE_LABELS = {
  preparing_targets: "Preparing targets", connectivity: "TCP/445 check",
  inspection: "SMB and content scan", authentication: "Authentication",
  share_discovery: "Share discovery", file_inventory: "File inventory",
  content_scan: "Content scan", identity_access: "Identity access",
  cancelling: "Cancelling", cancelled: "Cancelled",
  completed: "Completed", failed: "Failed",
};
const EN_SCAN_STATUS_LABELS = {idle: "No scan", running: "Running", cancelling: "Cancelling", cancelled: "Cancelled", completed: "Completed", failed: "Failed"};
const EN_STATUS_MESSAGES = {
  idle: "No scan has been started.", cancelling: "Cancelling scan.",
  cancelled: "Scan cancelled.", completed: "Scan completed.", failed: "Scan failed.",
};
const DETAIL_LABELS = {
  "Kimlik doğrulama": "Authentication", "Son durum": "Final status",
  "Denenen share": "Shares probed", "Erişilen share": "Shares accessible",
  "Görülen dosya": "Files seen", "Taranan dosya": "Files scanned",
  "Okunamayan dosya": "Unreadable files", "Hata ayrıntısı": "Error details",
  "Ayrıntı için bir hedef seç.": "Select a target to view details.",
  "Hedef": "Target", "Path": "Path", "Tür": "Type", "Durum": "Status",
  "Okuma": "Read access", "Yazma": "Write access", "Boyut": "Size",
  "Değiştirilme": "Modified", "Eşleşme": "Match", "Satır içeriği": "Line content",
  "Eşleşme çevresi": "Match context", "İçerikler sekmesinde aç": "Open in Contents",
  "Satır no": "Line number", "Yöntem": "Method", "Kural": "Rule",
  "Kategori": "Category",
  "Kaynak": "Source", "Bulgu": "Finding",
  "Bulgu sınıfı": "Finding class", "Eşleşme gücü": "Match strength",
};

function localizedMap(map, englishMap, key) {
  return currentLanguage === "en" ? englishMap[key] : map[key];
}
const LANGUAGE_TEXT = {
  en: {
    "Yeni tarama": "New scan",
    "Bekliyor": "Idle",
    "IP, CIDR veya hostname": "IP, CIDR, or hostname",
    "Tarama adı": "Scan name",
    "Virgül veya yeni satırla ayır.": "Separate with commas or new lines.",
    "Kimlik bilgisi": "Credentials",
    "Kullanıcı": "Username",
    "Kullanıcı (isteğe bağlı)": "Username (optional)",
    "Credential türü": "Credential type",
    "Parola": "Password",
    "CCache dosyası": "CCache file",
    "Kimlik doğrulama": "Authentication",
    "Yalnız Kerberos": "Kerberos only",
    "Yalnız NTLM": "NTLM only",
    "Auto (Önerilen)": "Auto (Recommended)",
    "İçerik arama": "Content search",
    "Terim kapsamı": "Term coverage",
    "Varsayılan TXT terim listesi": "Default TXT term list",
    "Seçilen TXT listesindeki kelime ve ifadeleri ara": "Search words and phrases in the selected TXT list",
    "Gizli değer ve kimlik kalıpları": "Secret and credential patterns",
    "Temel — temel kimlik bilgileri": "Basic — core credentials",
    "Orta — genişletilmiş servisler": "Balanced — extended services",
    "Fazla — tam liste": "Thorough — full list",
    "Birbirinden bağımsız üç TXT listesinden birini seçer.": "Selects one of three independent TXT lists.",
    "Düzenlenecek liste": "List to edit",
    "Temel": "Basic",
    "Orta": "Balanced",
    "Fazla": "Thorough",
    "Yalnız belirli dosyaları tara": "Scan only specific files",
    "İsteğe bağlı": "Optional",
    "Dosya adı veya yol kalıbı": "File name or path pattern",
    "Virgül veya yeni satırla ayır. Boşsa tüm yollar kapsanır.": "Separate with commas or new lines. Leave empty to include all paths.",
    "Örnek: *.xlsx, Muhasebe\\*, *password*. Boşsa tüm yollar kapsanır.": "Example: *.xlsx, Accounting\\*, *password*. Leave empty to include all paths.",
    "Dosya uzantıları": "File extensions",
    "Boşsa seçilen yoğunluğun güvenli varsayılanı kullanılır.": "Leave empty to use the selected intensity's safe default.",
    "Örnek: .xlsx, .docx, .txt. Boşsa tüm uzantılar kapsanır.": "Example: .xlsx, .docx, .txt. Leave empty to include all extensions.",
    "En büyük dosya (MiB)": "Maximum file size (MiB)",
    "Boşsa 32 MiB güvenlik sınırı kullanılır.": "Leave empty to use the 32 MiB safety limit.",
    "Share başına en fazla 2.500 eşleşen dosya taranır.": "At most 2,500 matching files are scanned per share.",
    "Profile göre": "From profile",
    "Varsayılan terim listesini kullan": "Use the default term list",
    "Varsayılan terimler": "Default terms",
    "Listeyi aç": "Open list",
    "TXT içeriğini görüntüleyebilir, düzenleyebilir veya başka bir TXT ile değiştirebilirsin.": "View or edit the TXT content, or replace it with another TXT file.",
    "İçerik arama terimleri": "Content search terms",
    "Kaydet": "Save",
    "— kayıt": "— entries",
    "Yükleniyor": "Loading",
    "Liste yüklenemedi": "Could not load the list",
    "Kaydediliyor": "Saving",
    "Kaydedildi": "Saved",
    "Liste kaydedilemedi": "Could not save the list",
    "İçe aktarıldı · henüz kaydedilmedi": "Imported · not saved yet",
    "Özel arama terimleri (isteğe bağlı)": "Custom search terms (optional)",
    "Özel arama terimleri": "Custom search terms",
    "TXT içe aktar": "Import TXT",
    "Terim üret": "Generate terms",
    "Kuruma özel terim üret": "Generate organization-specific terms",
    "Kuruma özgü adları girin. Parola, token ve API anahtarı terimleri özel arama terimlerine eklenir.": "Enter organization-specific names. Password, token, and API key terms are added to the custom search terms.",
    "Kuruma özgü adlar": "Organization-specific names",
    "Birden fazla adı virgül veya yeni satırla ayırın.": "Separate multiple names with commas or new lines.",
    "Terimleri ekle": "Add terms",
    "Otomatik Kural Tespiti": "Automatic Rule Detection",
    "Kurallar İle Otomatik İçerik Tespiti": "Automatic Rule-Based Content Detection",
    "Tespit kuralı paketleri": "Detection rule packs",
    "Genel kimlik bilgileri ve gizli değerler": "General credentials and secrets",
    "Windows ve Active Directory verileri": "Windows and Active Directory material",
    "Parola ve kimlik doğrulama hash'leri": "Password and authentication hashes",
    "Bulut, SaaS ve kaynak kod servisleri": "Cloud, SaaS, and source control services",
    "Altyapı, konteyner ve geliştirici araçları": "Infrastructure, container, and developer tools",
    "En az bir tespit kuralı paketi seç.": "Select at least one detection rule pack.",
    "Varsayılan terimleri veya otomatik tespiti etkinleştirin ya da en az bir özel terim girin.": "Enable default terms or automatic detection, or enter at least one custom term.",
    "Kapalı": "Off",
    "Yazma testleri": "Write tests",
    "SMB yazma erişimini test et": "Test SMB write access",
    "AD yazma erişimini test et": "Test AD write access",
    "SMB yazma testi": "SMB write test",
    "AD yazma testi": "AD write test",
    "Yazma erişimi testi": "Write-access test",
    "Test edilmedi": "Not tested",
    "Tümü": "All",
    "Taramayı başlat": "Start scan",
    "İptal et": "Cancel",
    "Taramayı durdur": "Stop scan",
    "Durduruluyor…": "Stopping…",
    "Tarama yok": "No scan",
    "Yeni bir tarama başlatılmadı.": "No scan has been started.",
    "Envanter": "Inventory",
    "İçerikler": "Contents",
    "Bulgu": "Finding",
    "Faz": "Phase",
    "Hedefler": "Targets",
    "Kimlik erişimi": "Identity access",
    "AD kimlik erişimi": "AD identity access",
    "Girilen kimlikle doğrudan kullanılabilecek AD erişimlerini gösterir.": "Shows AD access that may be directly usable with the supplied identity.",
    "İncelenmedi": "Not inspected",
    "Henüz AD kimlik incelemesi yok.": "No AD identity inspection yet.",
    "Önce bir tarama başlatın.": "Start a scan first.",
    "Bulgular": "Findings",
    "SMB Erişimi": "SMB Access",
    "Tespitler": "Detections",
    "Durum": "Status",
    "Bekliyor": "Idle",
    "Okunabilir İçerik": "Readable Content",
    "Geçmiş": "History",
    "Sonuçları JSON indir": "Download results as JSON",
    "Görüntüle": "View",
    "Sil": "Delete",
    "Share'ler": "Shares",
    "Dizinler": "Directories",
    "Dosyalar": "Files",
    "Filtre": "Filter",
    "İçerik kaynağı": "Content source",
    "Tüm kaynaklar": "All sources",
    "Yalnızca İncele": "Review only",
    "Dosya / Nesne": "File / Object",
    "Konum / Alan": "Location / Attribute",
    "Boyut": "Size",
    "İncele": "Review",
    "İşaret": "Marker",
    "Henüz canlı içerik yok.": "No live content yet.",
    "İçeriğini görmek için bir kayıt seç.": "Select an entry to view its content.",
    "İçerik ayrıntısı": "Content details",
    "Son durum": "Final status",
    "Tarama girdileri ve tamamlanan sonuçlar bu tarayıcıda saklanır.": "Scan inputs and completed results are stored in this browser.",
    "Tarama girdileri": "Scan inputs",
    "Girdileri görmek için bir tarama seç.": "Select a scan to view its inputs.",
    "Sonuçları aç": "Open results",
    "Tarama ayarlarını göster": "Show scan settings",
    "Hedef listesi": "Target list",
    "Kimlik türü": "Credential type",
    "Kimlik doğrulama modu": "Authentication mode",
    "Girilen parola": "Submitted password",
    "Girilen NT hash": "Submitted NT hash",
    "CCache dosya adı": "CCache file name",
    "CCache dosya boyutu": "CCache file size",
    "Dosya yolu": "File path",
    "Tarayıcı tam dosya yolunu paylaşmaz.": "The browser does not expose the full file path.",
    "Özel terimler": "Custom terms",
    "Veri kalıpları": "Data patterns",
    "Dahil edildi": "Included",
    "Dahil edilmedi": "Not included",
    "Bu kayıtta saklanmadı.": "Not retained in this record.",
    "Göster": "Show",
    "Gizle": "Hide",
    "0 hedef": "0 targets",
    "0 kayıt": "0 entries",
    "0 bulgu": "0 findings",
    "— kayıt": "— entries",
    "Ayrıntı için bir hedef seç.": "Select a target to view details.",
    "Ayrıntı için bir kayıt seç.": "Select an entry to view details.",
    "Ayrıntı için bir bulgu seç.": "Select a finding to view details.",
    "Henüz tarama başlatılmadı.": "No scan has been started.",
    "Hedef sonuçları bekleniyor.": "Waiting for target results.",
    "Hedef durumları bekleniyor.": "Waiting for target statuses.",
    "Bu filtreyle eşleşen hedef yok.": "No targets match this filter.",
    "Henüz envanter yok.": "No inventory yet.",
    "Filtreyle eşleşen kayıt yok.": "No entries match this filter.",
    "Henüz bulgu yok.": "No findings yet.",
    "Filtreyle eşleşen bulgu yok.": "No findings match this filter.",
    "Henüz kayıtlı tarama yok.": "No saved scans yet.",
    "Kullanıcı yok": "No username",
    "Taramayı sil": "Delete scan",
    "Bu tarama geçmişten kalıcı olarak silinecek. Bu işlem geri alınamaz.": "This scan will be permanently deleted from history. This action cannot be undone.",
    "Vazgeç": "Cancel",
    "Geçmişten sil": "Delete from history",
    "Dosya": "File",
    "Dizin": "Directory",
    "Share": "Share",
    "Kayıt": "Entry",
    "kayıt": "entries",
    "bulgu": "findings",
    "Yanıt veren": "Responding",
    "TCP açık": "TCP open",
    "SMB hazır": "SMB ready",
    "Doğrulandı": "Authenticated",
    "Sorunlu": "Needs attention",
    "Tarama çalışma alanı": "Scan workspace",
    "Tarama ilerlemesi": "Scan progress",
    "Tarama sonuçları": "Scan results",
    "Hedef durumu filtreleri": "Target status filters",
    "Hedef ayrıntısı": "Target details",
    "Envanter ayrıntısı": "Inventory details",
    "Bulgu ayrıntısı": "Finding details",
    "Kapat": "Close",
    "Dosya içeriği": "File content",
    "Salt okunur önizleme": "Read-only preview",
    "İndir": "Download",
    "SMB sürümü": "SMB dialect",
    "İmzalama": "Signing",
    "Şifreleme": "Encryption",
    "En az bir kuruma özel ad girin.": "Enter at least one organization-specific name.",
    "Yeni terim yok.": "No new terms.",
    "Bağlantı": "Connection",
    "İptal": "Cancellation",
    "Yerel panel yanıt vermedi.": "The local dashboard did not respond.",
    "Yalnız .txt dosyası seçilebilir": "Only a .txt file can be selected",
    "TXT dosyası en fazla 1 MiB olabilir": "The TXT file can be at most 1 MiB",
    "TXT dosyası okunamadı": "The TXT file could not be read",
    "CCache dosyası seçilmelidir.": "Select a CCache file.",
    "Yalnız .ccache uzantılı dosya seçilebilir.": "Only a .ccache file can be selected.",
    "CCache dosyası boş olamaz.": "The CCache file cannot be empty.",
    "CCache dosyası en fazla 1 MiB olabilir.": "The CCache file can be at most 1 MiB.",
    "CCache dosyası okunamadı.": "The CCache file could not be read.",
  },
};

function uiText(value) {
  if (currentLanguage !== "en") return value;
  return LANGUAGE_TEXT.en[value] ?? DETAIL_LABELS[value] ?? value;
}

function numberLocale() {
  return currentLanguage === "en" ? "en-US" : "tr-TR";
}

function applyLanguage(language) {
  const dictionary = LANGUAGE_TEXT[language];
  if (!dictionary) return;
  document.documentElement.lang = language;
  const walker = document.createTreeWalker(document.body, NodeFilter.SHOW_TEXT);
  while (walker.nextNode()) {
    const node = walker.currentNode;
    const translated = dictionary[node.nodeValue.trim()];
    if (translated) node.nodeValue = node.nodeValue.replace(node.nodeValue.trim(), translated);
  }
  for (const attribute of ["placeholder", "aria-label", "title"]) {
    for (const element of document.querySelectorAll(`[${attribute}]`)) {
      const value = element.getAttribute(attribute);
      if (dictionary[value]) {
        element.setAttribute(attribute, dictionary[value]);
      }
    }
  }
}

export {
  EN_CATEGORY_LABELS,
  EN_FINDING_METHOD_LABELS,
  EN_FINDING_RULE_LABELS,
  EN_PHASE_LABELS,
  EN_SCAN_STATUS_LABELS,
  EN_STATUS_LABELS,
  EN_STATUS_MESSAGES,
  ERROR_MESSAGE_LABELS,
  FINDING_METHOD_LABELS,
  FINDING_RULE_LABELS,
  LANGUAGE_KEY,
  MESSAGE_LABELS,
  PHASE_LABELS,
  SCAN_STATUS_LABELS,
  STATUS_LABELS,
  STATUS_MESSAGES,
  TR_CATEGORY_LABELS,
  applyLanguage,
  currentLanguage,
  localizedMap,
  numberLocale,
  uiText,
};
